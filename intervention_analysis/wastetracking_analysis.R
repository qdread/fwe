...
ggplot(establishments_affected %>% ungroup %>% mutate(BEA_Title = factor(BEA_Title, levels = unique(BEA_Title))), aes(y = establishments, x = BEA_Title, fill = sector, alpha = use)) + 
  ...
  ggtitle('Number of establishments that can adopt waste tracking', 'based on having >20 employees and being in a subsector that involves providing food')
...
ggplot(proportion_affected %>% ungroup %>% mutate(BEA_Title = factor(BEA_Title, levels = unique(BEA_Title))), aes(y = final_proportion, x = BEA_Title, fill = sector)) + 
  ...
  scale_y_continuous(name = 'Proportion of impact affected', limits = c(0,1), expand = c(0,0)) +
  scale_fill_brewer(palette = 'Set2') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle('Proportion of impact by BEA industry that will be influenced by waste tracking', 'based on proportion of impact in each sector due to food, and excluding firms with <20 employees')
