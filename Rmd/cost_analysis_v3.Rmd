...
The resulting pattern of environmental impact is an asymptotic abatement curve because we are choosing sectors with successively less importance for the given impact category.
...
*Uncertainty in weighting parameters.* There are two types of weights being used. One is the proportion of each sector that is assigned to the FSC, and the other is the weight of each of the 13 food categories in each sector. These weights are derived from a variety of sources as described elsewhere in this document. It is likely that the food-nonfood proportion in each sector is much more important for the results than the relative proportions of food within each sector. Therefore, we are accounting for the uncertainty in the food-nonfood proportion weights but not the relative food category proportion weights.
...
* $C_{1}$: Cost of reducing waste to the diversion potential level. This cost value is the total for each stage, so for each sector, the cost value is multiplied by the proportion output within that stage represented by the sector. **Source: ReFED.**

Table 5 shows the values derived from the report for net waste $N$, addressable waste $A$, diversion potential $D$, and cost of reducing waste to the diversion potential level $C_1$.
...
Separate values for each parameter are calculated for each of the many sectors within each FSC stage. Figure 9 shows a representative cost curve for one sector from each food supply chain stage.
...
Figure 8 shows that for some of the sectors,the waste rate is reduced after a small initial investment, but others do not see much reduction until a large amount of money has been invested in waste reduction. Clearly this has a significant influence on the result of the optimization (see below). In addition, waste reduction has a very different maximal effectiveness in the different sectors, which will also determine prioritization of funding.
...
1. The amount of money invested in each stage (6 stages) is divided proportionately by sector size among the sectors making up each stage (around 10-30 sectors per stage). 
...
I ran a separate optimization for four different values of $C_{total}$: 500, 1000, 2000, and 5000, which would be divided among the 6 FSC stages and then equally among the sectors making up each stage. I corrected for sectors that are only partially involved in the food supply chain (for example, food system sectors account for about 12% of inputs to the amusement park industry. Making the simplifying assumption that this corresponds to 12% of final demand for the amusement park industry being food purchases, the final demand modification for the industry only modifies 12% of its total final demand.) For each value of $C_{total}$, I optimized for minimizing GHG emissions, land use, water use, and energy use, for a total of 16 optimizations.

## Results of optimization

Taking GHG emissions as an example, the optimal waste rate across all sectors that is achieved with the cost allocation that minimizes emissions shows interesting patterns as total available FLW reduction funds change (Figure 10, top right-hand panel). For lower total investment in FLW reduction, the lowest total GHG emissions of the food system is achieved by investing most in FLW reduction at the food service and agricultural production stages, later followed by household and institutional food service stages, which are relatively more expensive to implement. The processing and retail sectors are modeled as being cheap to implement but with relatively low waste reduction, so it is optimal to invest relatively less in FLW reduction in those sectors.
...
In addition to the baseline waste rate parameter $W_0$ which is ultimately derived from FAO data, there are three other parameters currently derived from ReFED data that we are also incorporating into the sensitivity analysis. They are $W_u$, the unavoidable waste rate, $W_1$, the intermediate waste rate, and $C_1$, the cost to reduce waste to $W_1$. Since $W_u$ and $W_1$ are proportions we are using beta distributions for the sensitivity analysis. Since $C_1$ is a quantity we are using a triangular distribution with upper and lower bounds equal to $[\frac{1}{2}C_1, \frac{3}{2}C_1]$. 

Because it is necessary that $W_0 > W_1 > W_u$, we used truncated beta distributions to ensure that the inequality holds for each sample draw.

### Monte Carlo sensitivity analysis

As we did for the cost-free reduction scenarios above, we took 100 draws from the distributions for baseline waste rate, food-nonfood proportion for each sector, and all the cost curve parameter values derived from ReFED. We ran the entire set of optimizations 100 times with each of the replicate parameter sets. The error bars on the optimization figures are the 2.5% and 97.5% quantiles from the replicated optimization results.

### Results of sensitivity analysis

*add content here when it is finished running*
