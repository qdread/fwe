...
An alternative way to show the tradeoffs among impact categories resulting from where we reduce waste is shown in the following "radar chart." In this chart, points further from the center indicate greater *reduction* of impacts in that category. 
...
Separate values for each parameter are calculated for each of the many sectors within each FSC stage. Figure 9 shows the average cost curve for each of the six stages, averaging together all of the sectors within each stage.

![Average abatement cost curve for each supply chain stage](Q:/figures/sixstage_costcurve_refedcurvesbystage.png){ width=50% }

Figure 9 shows that using this method of finding the parameters of the curves, the waste reduction rate flattens out relatively quickly and is relatively modest. As might be expected, this leads to low estimates of impact reduction given investment of food waste reduction (see below).
