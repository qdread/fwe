...
If we simulate reduction in FLW in a particular sector, say production, this has two different impacts on the structure of the use table. For instance, say bread production, a sector with 100% FSC-related output, has a baseline loss rate of 20%. If we reduce the loss rate by 25% in that sector, we have a new loss rate of 15%. Using the equation above, the bread sector now demands 94.1% of the amount of inputs to produce the same amount of outputs (i.e., it is more efficient due to the reduction in FLW). We modify the use table accordingly by multiplying all the intermediate inputs to the bread production sector by 0.941. In addition, we multiply the **this paragraph is incomplete -- I will revise this once I come up with the newer scenario analysis stuff.**
