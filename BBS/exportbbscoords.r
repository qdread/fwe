...
bbscoords <- read.csv('/nfs/fwe-data/BBS/bbs_stop_coords.csv')
write.table(bbscoords[1:10,c('lon','lat')], row.names = FALSE, col.names = FALSE, file = '/nfs/fwe-data/BBS/test.txt')
write.table(bbscoords[,c('lon','lat')], row.names = FALSE, col.names = FALSE, file = '/nfs/fwe-data/BBS/stopcoords.txt')

# run getnlcd.sh then load the extracted NLCD values and combine them with the coordinates.
bbsstopnlcd <- read.table('/nfs/fwe-data/BBS/stopnlcd.txt', header = FALSE)
...
library(raster)
n16 <- raster('/nfs/fwe-data/landuse/NLCD/NLCD_2016_Land_Cover_L48_20190424.img')
...
write.csv(bbscoords_nlcd, file = '/nfs/fwe-data/BBS/bbs_stop_coords_nlcd.csv', row.names = FALSE)