...
get_eeio_result_sensitivity <- function(input, c_factor, r_factor, c_names, r_names, i = 'no_name', crosswalk = naics_foodsystem) {
  ...
  crosswalk$proportion_food <- input$proportion_food
...
n_draws <- 99 # Number of rounds of sensitivity analysis
...
write.csv(grid_sensitivity_CIs, file.path(fp_output, 'sensitivity_grid.csv'), row.names = FALSE)
